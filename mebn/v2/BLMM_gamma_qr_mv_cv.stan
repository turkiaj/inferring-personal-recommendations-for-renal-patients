...
transformed data { 
  ...
  for (i in 1:v)
  {
    ...
    Q_ast[i] = qr_thin_Q(X_t) * sqrt(N-NH - 1);
    R_ast[i] = qr_thin_R(X_t) / sqrt(N-NH - 1);
...
model {
  ...
  {
    vector[N-NH] mu;             // expected value 
    vector[N-NH] g_beta;         // beta (rate) of Gamma distribution
