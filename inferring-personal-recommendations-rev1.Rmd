...
ggsave("figures/ppc_mv3_cross.pdf", plot = p, device = "pdf", width = 4, height = 4)

# Save TIFF. This only works from console.
library(Cairo)
Cairo(file = "figures/Fig4.tiff", type = "tiff", width = 4, height = 4, bg="white", units = "in", res = 300)
print(p)
dev.off()
...
top_effect_summary <- effect_summary %>%
  ...
  select(effect, personal_min, personal_max, lCI_min, uCI_min, lCI_max, uCI_max, personal_abs, personal_variation, sigma_g, sigma_g_lCI, sigma_g_uCI, sigma_b, sigma_b_lCI, sigma_b_uCI) %>%
  distinct()
...
kable(top_effect_summary[1:20, c(16,2,3,10,13)], "latex", escape = T, booktabs = T, row.names = FALSE, digits = 3, col.names = c("Effect", "Study min.", "tudy max.","Sigma_g","Sigma_b")) %>%
    kable_styling(latex_options = "HOLD_position") %>%
    kable_styling(latex_options = c("basic", "condensed"), full_width = TRUE) %>%
    add_header_above(c(" " = 1, "Expected personal effect strength" = 2, "Inter-treatment/patient variation"), escape = F) %>%
    row_spec(0,bold=TRUE) %>%
...
pho_plot <- ggplot(intake_reasoning.df, aes(x = pho, y = reorder(person_id, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000), group = person_id)) +
      ...
      ggtitle(expression(paste("Phosp. (mg/d)          ", symbol('\256'))))
...
#recommendation_plot <- grid.arrange(grobs=list(pot_plot,pho_plot,pk_plot,fppi_plot,palb_plot),top=textGrob(expression(underline("Personally recommended intake ranges and corresponding concentration predictions"))),ncol=5,nrow=1,padding=unit(0,"cm"),widths=c(1.32,1,1,1,1))
...
#library(Cairo)
#Cairo(file = "figures/Fig3.tiff", type = "tiff", width = 6, height = 6, bg="white", units = "in", res = 400)
...
pot_plot <- ggplot(intake_reasoning.df, aes(x = pot, y = reorder(person_text, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000), group = person_id)) +
      ...
      theme_bw() +
      scale_x_continuous(breaks=c(0,2500,5800), limits = c(0,5850)) +
      geom_text(stat = "unique", aes(x=2500, label=pot_text), nudge_y=0.32, size=2.1) +
      geom_vline(xintercept = 2500, linetype="solid", color = "black", size=0.2) +
      labs(x = element_blank(), y = "Patients") +
      theme(axis.text.x = element_text(size=6),axis.title = element_text(size=9), axis.text = element_text(size=7), plot.title = element_text(size=8)) +
      ggtitle("Pot. (mg/d)")
...
pho_plot <- ggplot(intake_reasoning.df, aes(x = pho, y = reorder(person_id, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000), group = person_id)) +
      ...
      theme_bw() +
      scale_x_continuous(breaks=c(0,1000,2550), limits = c(0,3000)) +
      geom_text(stat = "unique", aes(x=1000, label=pho_text), nudge_y=0.3, size=2.1) +
      geom_vline(xintercept = 1000, linetype="solid", color = "black", size=0.2) +
      labs(x = element_blank(), y = element_blank()) +
      theme(axis.text.x = element_text(size=6),axis.title = element_text(size=9), axis.text.y = element_blank(), plot.title = element_text(size=8)) +
      ggtitle("Phosp. (mg/d)   →")
...
pk_plot <- ggplot(data = conc_reasoning.df, aes(y = reorder(person_id, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000)), group = accepted) +
      ...
      #geom_point(aes(x=pk_upper_limit), shape=3) +
      labs(x = element_blank(), y = element_blank()) +
      theme(axis.text.y = element_blank(), axis.text.x = element_text(size=6), axis.title = element_text(size=9), plot.title = element_text(size=8)) +
      ggtitle("P-K (mmol/l)")
...
#library(Cairo)
#Cairo(file = "figures/Fig3.tiff", type = "tiff", width = 6, height = 6, bg="white", units = "in", res = 600)
#grid.arrange(grobs=list(pot_plot,pho_plot,pk_plot,fppi_plot,palb_plot),top=textGrob("Personal intake recommendations                        Corresponding #concentration predictions", gp = gpar(fontsize = 9)),ncol=5,nrow=1,padding=unit(0,"cm"),widths=c(1.45,1,1,1,1))
#dev.off()
  ...
  if (file.exists(accepted_intake)) {
    ...
    else
    {
      ...
      intake_plot <- ggplot(subset(accepted_intake.df, lowest_conc_prob >= 0.50), aes(x = pot, y = pho)) +
        ...
        theme(legend.position="bottom", axis.text = element_text(size=6), axis.title = element_text(size=8), plot.title = element_text(size=8)) + 
        ...
        geom_segment(aes(xend = avg_k+60, x = 3550, yend = avg_f-40, y = 100), color="#333333", size=0.15) +
        geom_segment(aes(xend = 5400, x = 4000, yend = 250, y = 250), color="#333333", size=0.15, arrow = arrow(type = "closed", length = unit(0.06, "inches"))) +
        annotate("label", x = 3500, y = 100, label = "Observed intake that produced\nthe concentrations with dashed lines", size=2.5, vjust = "bottom")    
...
# Saving to TIFF only works from console
#library(Cairo)
#Cairo(file = "figures/Fig2.tiff", type = "tiff", width = 6, height = 6, bg="white", units = "in", res = 400)
