...
dialysis <- read.csv(file="data/DIALYSIS_DATES_OMITTED.csv", sep=";", dec=",")
...
# Omit the dates and store only the difference
...
#dialysis$labraaika <- "removed"
#dialysis$ravhaastaika <- "removed"
#write.csv2(dialysis, file="data/DIALYSIS_DATES_OMITTED.csv", row.names=FALSE, sep=";", dec=",")
#dialysis1 <- read.csv(file="data/DIALYSIS.csv", sep=";", dec=",")
#dialysis2 <- read.csv(file="data/DIALYSIS_DATES_OMITTED.csv", sep=";", dec=",")
#dialysis2
...
# Saving to TIFF only works from console
library(Cairo)
Cairo(file = "figures/FigS1.tiff", type = "tiff", width = 4, height = 4, bg="white", units = "in", res = 300)
grid.arrange(pk_k_plot, pk_p_plot, fppi_k_plot, fppi_p_plot, palb_k, palb_p, nrow = 3, ncol=2, padding=0)
dev.off()
```
...
dialysis_imputed$potilas <- as.factor(dialysis_imputed$potilas)
...
# non-centered version: stan_model_file = "mebn/v2/BLMM_gamma_two_level_grouping.stan",
...
dialdiet_gamma_mv3_two_level <- mebn.bipartite_two_level_multivariate(
                                   ...
                                   stan_model_file = 
                                     "mebn/v2/BLMM_gamma_two_level_centered.stan",
                                   normalize_values = TRUE)

write.graph(dialdiet_gamma_mv3_two_level, "graphs/dialysis_gamma_two_level_centered.graphml", "graphml")
```
...
latent_parameter_modeldir <- "models/BLMM_gamma_mv_cross/two_levels/noncentered/"
...
for (person_id in 2:37) {
  ...
  # - pick rows for the selected person
  subject_code <- patients[person_id]
  personal_data_df <- cbind(dialysis$potilas, normalized_input_df)
  personal_data_df <- personal_data_df[personal_data_df$`dialysis$potilas` == subject_code,]
  personal_data <- as.matrix(subset(personal_data_df, select = -c(`dialysis$potilas`)))
...
cv_pk_mse <- mean(as.numeric(cv_mse_summary[cv_mse_summary$reaction_model == "mv3_cross_two_levels_cv",]$pk_nrmse)) # 0.1696225
cv_fppi_mse <- mean(as.numeric(cv_mse_summary[cv_mse_summary$reaction_model == "mv3_cross_two_levels_cv",]$fppi_nrmse)) # 0.2310911
cv_palb_mse <- mean(as.numeric(cv_mse_summary[cv_mse_summary$reaction_model == "mv3_cross_two_levels_cv",]$palb_nrmse), na.rm=TRUE) # 0.06320297
...
mv3cv <- cbind("mv3_cross_two_levels_cv", cv_pk_mse, cv_fppi_mse, cv_palb_mse, mean(cv_pk_mse, cv_fppi_mse, cv_fppi_mse))

mse_table <- rbind(mv3c2, mv3cv, mv3c1, unic1)
...
```{r dialysis_gamma_mv_cross_2levels_ppc, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE, cache=TRUE, fig.cap="Posterior predictive check of the model where three concentrations and their parameters are stacked into one univariate model for estimating cross-model correlations."}
source("mebn/v2/MEBNv2.r")
dialysis_imputed <- readRDS("data/DIALYSIS_imputed_palb.rds")
pk_fppi_palb_targets <- datadesc_fat_epros[datadesc_fat_epros$Name %in% c('pk','fppi','palb'),]
...
p <- mebn.multivariate_dens_overlays("models/BLMM_gamma_mv_cross/two_levels/noncentered", pk_fppi_palb_targets, dialysis_imputed) 

#ggsave("figures/Fig5.pdf", plot = p, device = "pdf", width = 4, height = 4)

# Save TIFF. This only works from console.
library(Cairo)
Cairo(file = "figures/Fig5.tiff", type = "tiff", width = 4, height = 4, bg="white", units = "in", res = 400)
print(p)
dev.off()
...
Here we look into the estimated correlation matrices of effects. The model estimates how much the effects of nutrients vary between different types of dialysis treatment and further how the effects vary personally within a treatment. These correlation plots show how the effects are correlated. 
...
target_blmm <- mebn.get_localfit("pk_fppi_palb", "models/BLMM_gamma_mv_cross/two_levels/noncentared")
...
# Saving to TIFF only works from console
# Cairo(file = "figures/FigS2.tiff", type = "tiff", width = 11, height = 11, bg="white", units = "in", res = 300)
# print(cplot)
# dev.off()
...
# Saving to TIFF only works from console
# Cairo(file = "figures/FigS3.tiff", type = "tiff", width = 11, height = 11, bg="white", units = "in", res = 300)
# print(cplot)
# dev.off()
...
```{r, include=TRUE, fig.align="center", fig.cap=c("Correlation plot of matrix $C_g$ that includes the estimated within-model and cross-model correlations in level of dialysis treatment. The figure is plotted with ggcorrplot package for R language (v 0.1.4, https://cran.r-project.org/web/packages/ggcorrplot)."), echo=FALSE}
knitr::include_graphics("figures/treatment_level_reaction_correlations.pdf")
```

```{r, include=TRUE, fig.align="center", fig.cap=c("Correlation plot of matrix $C_b$ that includes the estimated within-model and cross-model correlations of personal effects. The figure is plotted with ggcorrplot package for R language (v 0.1.4, https://cran.r-project.org/web/packages/ggcorrplot)."), echo=FALSE}
...
effect_cor.df$effect1 <- unlist(lapply(strsplit(effect_cor.df$effect1, ":"), function(x) ifelse(is.na(x[2]), x[1], paste0(x[2]," $\rightarrow$ ",x[1]))))
effect_cor.df$effect2 <- unlist(lapply(strsplit(effect_cor.df$effect2, ":"), function(x) ifelse(is.na(x[2]), x[1], paste0(x[2]," $\rightarrow$ ",x[1]))))
...
kable(rbind(top_pot_correlations, top_pho_correlations), "latex", escape = T, booktabs = T, row.names = FALSE, digits = 3, col.names = c("Effect 1", "Effect 2", "Correlation"), caption = "Table shows 40 highest positive or negative correlations between potassium and phosphorous treatment effects with other treatment effects. This structure of correlations is used in
estimating the personal effects based on personal intake and matching concentrations.") %>%
    kable_styling(latex_options = "HOLD_position") %>%
...
effect_summary$sigma_b_lCI <- as.numeric(effect_summary$sigma_b_lCI)
effect_summary$sigma_b_uCI <- as.numeric(effect_summary$sigma_b_uCI)
effect_summary$sigma_g <- as.numeric(effect_summary$sigma_g)
effect_summary$sigma_g_lCI <- as.numeric(effect_summary$sigma_g_lCI)
effect_summary$sigma_g_uCI <- as.numeric(effect_summary$sigma_g_uCI)
...
top_effect_summary <- effect_summary %>%
  ...
  mutate(personal_max = personal_effect[which.max(personal_effect)],
         personal_min = personal_effect[which.min(personal_effect)],
         lCI_max = personal_effect_lCI[which.max(personal_effect)],
         uCI_max = personal_effect_uCI[which.max(personal_effect)],
         lCI_min = personal_effect_lCI[which.min(personal_effect)],
         uCI_min = personal_effect_uCI[which.min(personal_effect)],
         sigma_g = sigma_g[which.min(personal_effect)],
         sigma_g_lCI = sigma_g_lCI[which.min(personal_effect)],
         sigma_g_uCI = sigma_g_uCI[which.min(personal_effect)],
         sigma_b = sigma_b[which.min(personal_effect)],
         sigma_b_lCI = sigma_b_lCI[which.min(personal_effect)],
         sigma_b_uCI = sigma_b_uCI[which.min(personal_effect)],
         general_effect = general_effect[which.min(personal_effect)],
         general_effect_lCI = general_effect_lCI[which.min(personal_effect)],
         general_effect_uCI = general_effect_uCI[which.min(personal_effect)],
         personal_abs = max(abs(personal_min), abs(personal_max))) %>% 
  mutate(personal_variation = personal_max - personal_min) %>% 
  arrange(desc(personal_abs), desc(personal_variation)) %>%
  select(effect, personal_min, personal_max, lCI_min, uCI_min, lCI_max, uCI_max, personal_abs, personal_variation, sigma_g, sigma_g_lCI, sigma_g_uCI, sigma_b, sigma_b_lCI, sigma_b_uCI,general_effect,general_effect_lCI,general_effect_uCI) %>%
  distinct()
...
#top_effect_summary <- top_effect_summary %>% inner_join(effect_summary, by = "effect")
d <- 2

#top_effect_summary$general <- paste0(round(as.numeric(top_effect_summary$general_effect),d), " [", round(as.numeric(top_effect_summary$general_effect_lCI), d), "; ", round(as.numeric(top_effect_summary$general_effect_uCI), d), "]")

top_effect_summary$general <- round(as.numeric(top_effect_summary$general_effect),d)

top_effect_summary$general_CI <- paste0("[", round(as.numeric(top_effect_summary$general_effect_lCI), d), "; ", round(as.numeric(top_effect_summary$general_effect_uCI), d), "]")

#top_effect_summary$personal_min <- paste0(round(as.numeric(top_effect_summary$personal_min),d), " [", round(as.numeric(top_effect_summary$lCI_min), d), "; ", round(as.numeric(top_effect_summary$uCI_min), d), "]")

top_effect_summary$personal_min <- paste0(round(as.numeric(top_effect_summary$personal_min),d))

top_effect_summary$personal_min_CI <- paste0("[", round(as.numeric(top_effect_summary$lCI_min), d), "; ", round(as.numeric(top_effect_summary$uCI_min), d), "]")

#top_effect_summary$personal_max <- paste0(round(as.numeric(top_effect_summary$personal_max),d), " [", round(as.numeric(top_effect_summary$lCI_max), d), "; ", round(as.numeric(top_effect_summary$uCI_max), d), "]")

top_effect_summary$personal_max <- paste0(round(as.numeric(top_effect_summary$personal_max),d))

top_effect_summary$personal_max_CI <- paste0("[", round(as.numeric(top_effect_summary$lCI_max), d), "; ", round(as.numeric(top_effect_summary$uCI_max), d), "]")

#top_effect_summary$sigma_g <- paste0(round(as.numeric(top_effect_summary$sigma_g),d), " [", round(as.numeric(top_effect_summary$sigma_g_lCI), d), "; ", round(as.numeric(top_effect_summary$sigma_g_uCI), d), "]")

top_effect_summary$sigma_g <- paste0(round(as.numeric(top_effect_summary$sigma_g),d))

top_effect_summary$sigma_g_CI <- paste0("[", round(as.numeric(top_effect_summary$sigma_g_lCI), d), "; ", round(as.numeric(top_effect_summary$sigma_g_uCI), d), "]")

#top_effect_summary$sigma_b <- paste0(round(as.numeric(top_effect_summary$sigma_b),d), " [", round(as.numeric(top_effect_summary$sigma_b_lCI), d), "; ", round(as.numeric(top_effect_summary$sigma_b_uCI), d), "]")

top_effect_summary$sigma_b <- paste0(round(as.numeric(top_effect_summary$sigma_b),d))

top_effect_summary$sigma_b_CI <- paste0("[", round(as.numeric(top_effect_summary$sigma_b_lCI), d), "; ", round(as.numeric(top_effect_summary$sigma_b_uCI), d), "]")

```

```{r}
top_effect_summary
top_effect_summary[1:20,c(19,22,23,2,24,3,25,10,26)]

```

```{r top_effect_summary_table,echo=FALSE, cache=FALSE}
# Table shows 20 effects that have highest variation between patients by comparing minimum and maximum of expected effect strength.

kable(top_effect_summary[1:20, c(19,22,23,2,24,3,25,10,26)], "latex", escape = T, booktabs = T, row.names = FALSE, digits = 3, col.names = c("Effect", "General effect","CI","Study min.","CI","Study max.", "CI", "Sigma_g", "CI")) %>%
    kable_styling(latex_options = "HOLD_position") %>%
    kable_styling(latex_options = c("basic", "condensed"), full_width = TRUE) %>%
    add_header_above(c(" " = 1, "Expected effect strength" = 6, "Inter-treatment variation" = 2), escape = F) %>%
    row_spec(0,bold=TRUE) %>%
    save_kable(file = "tables/top_effect_summary_table2.pdf", keep_tex = TRUE)
...
kable(top_effect_summary[1:20, c(19,22,2,3,10)], "latex", escape = T, booktabs = T, row.names = FALSE, digits = 3, col.names = c("Effect", "General effect" ,"Study min.", "Study max.","Sigma_g")) %>%
    kable_styling(latex_options = "HOLD_position") %>%
    kable_styling(latex_options = c("basic", "condensed"), full_width = TRUE) %>%
    add_header_above(c(" " = 2, "Expected personal effect strength" = 2, "Inter-treatment variation"), escape = F) %>%
    row_spec(0,bold=TRUE) %>%
    save_kable(file = "tables/top_effect_summary_table.pdf", keep_tex = TRUE)
...
intake_reasoning.df[intake_reasoning.df$min_pot == "-1",]$pot_text <- ""
...
pot_plot <- ggplot(intake_reasoning.df, aes(x = pot, y = reorder(person_text, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000), group = person_id)) +
      ...
      geom_text(stat = "unique", aes(x=2500, label=pot_text), nudge_y=0.32, size=1.8) +
      geom_vline(xintercept = 2500, linetype="solid", color = "black", size=0.1) +
      labs(x = element_blank(), y = "Patients") +
      theme(text = element_text(family = "Times New Roman")) +      
      theme(axis.text.x = element_text(size=7),axis.title = element_text(size=9), axis.text = element_text(size=7), plot.title = element_text(size=9)) +
      # TIF: theme(axis.text.x = element_text(size=5.5),axis.title = element_text(size=8), axis.text = element_text(size=5.5), plot.title = element_text(size=8)) +
      ggtitle("Pot. (mg/d)")

      # PDF: theme(axis.text.x = element_text(size=7),axis.title = element_text(size=9), axis.text = element_text(size=7), plot.title = element_text(size=9)) +
      # TIF: theme(axis.text.x = element_text(size=5.5),axis.title = element_text(size=8), axis.text = element_text(size=5.5), plot.title = element_text(size=8)) +
...
pho_plot <- ggplot(intake_reasoning.df, aes(x = pho, y = reorder(person_id, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000), group = person_id)) +
      ...
      geom_text(stat = "unique", aes(x=1000, label=pho_text), nudge_y=0.3, size=1.8) +
      geom_vline(xintercept = 1000, linetype="solid", color = "black", size=0.1) +
      labs(x = element_blank(), y = element_blank()) +
      theme(axis.text.x = element_text(size=7),axis.title = element_text(size=9), axis.text.y = element_blank(), axis.text = element_text(size=7), plot.title = element_text(size=9)) +
      # TIF: theme(axis.text.x = element_text(size=5.5),axis.title = element_text(size=8), axis.text.y = element_blank(), axis.text = element_text(size=5.5), plot.title = element_text(size=8)) +
      theme(text = element_text(family = "Times New Roman")) +
      ggtitle("Phosp. (mg/d)")
...
save_pdf_plot <- function(filename, width, height) {
  ...
  pushViewport(vp)
  grid.segments(x0 = 0.79, x1 = 0.93, y0 = 0.5, y1 = 0.5, arrow = arrow(type = "closed", length = unit(0.06, "inches")), gp = gpar(fill = "black", col = "black"))
...
#save_tiff_plot(filename = "figures/Fig4.tif", width = 5, height = 5)

save_pdf_plot(filename = "figures/Fig4.pdf", width = 6, height = 6)

# ggsave(paste0("figures/Fig4.pdf"), plot = recommendation_plot, width = 6, height = 6, scale = 1)
# 
# library(Cairo)
# Cairo(file = "figures/Fig4.tiff", type = "tiff", width = 6, height = 6, bg="white", units = "in", res = 600)
# grid.arrange(grobs=list(pot_plot,pho_plot,pk_plot,fppi_plot,palb_plot),top=textGrob("Personal intake recommendations                        Corresponding concentration redictions", gp = gpar(fontsize = 9)),ncol=5,nrow=1,padding=unit(0,"cm"),widths=c(1.45,1,1,1,1))
# dev.off()

# This vector is used to arrange the predicted recommendation is the same order for easy comparison

person_id_reorder <- conc_reasoning.df$max_recom_norm + as.numeric(conc_reasoning.df$accepted_recommandation_probability) * 100000
person_ordering.df <- data.frame(person_id = conc_reasoning.df$person_id, person_order = person_id_reorder)
saveRDS(person_ordering.df, "patient_summary/person_ordering.df.rds")
 
```


```{r nuts_intake_method_figure, cache=FALSE, eval=FALSE, echo=FALSE,message=FALSE, warning=FALSE, fig.cap="Figure shows detailed intake recommendation for patient 10 from the previous recommendation table. The intake plot on the left shows the posterior samples of diet configurations that result concentrations of plasma potassium (P-K), fasting plasma phosphate (fP-Pi) and plasma albumin (P-Alb) to stay within their recommended limits. These levels are marked with vertical solid lines in the concentration panels. The black point in the middle of intake plot represents the patient's current potassium and phosphorous intake. Current concentrations matching this intake are shown with dashed vertical lines. Reported recommendation is shown with a rectangle that contains $95\\%$ of diet proposals that result recommended concentrations over $90\\%$ accuracy. Colouring of the rectangle sides match the concentration estimates in the right hand panels. The figure is plotted with ggplot2 package for R language (v 3.4.1, https://ggplot2.tidyverse.org)."}
library(ggplot2)
library(gridExtra)
library(ggrepel)
  ...
  if (file.exists(accepted_intake)) {
    ...
    if (patient_summary$min_pot == -1)
    {
        intake_plot <- ggplot(accepted_intake.df, aes(x = pot, y = pho)) +
          ...
          guides(colour = guide_colorbar(title = "Probability of reaching target concs.", title.theme = element_text(size=9), title.position = "top", barwidth = unit(80,"mm"))) 
    ...
    else
    {
      ...
      intake_plot <- ggplot(subset(accepted_intake.df, lowest_conc_prob >= 0.50), aes(x = pot, y = pho)) +
        geom_point(aes(colour=lowest_conc_prob), show.legend = TRUE) +
        theme(legend.position="bottom", axis.text = element_text(size=7), axis.title = element_text(size=8), plot.title = element_text(size=8)) + 
        ...
        geom_segment(color = MyPalette[1], size = 0.6, show.legend = TRUE,
                    aes(recommendation = "Min potassium",
                    x = as.numeric(personal_recommendation.df$min_pot),
                    y = as.numeric(personal_recommendation.df$min_pho),
                    xend = as.numeric(personal_recommendation.df$max_pot),
                    yend = as.numeric(personal_recommendation.df$min_pho))) +
        geom_segment(color = MyPalette[2], size = 0.6,
                    aes(recommendation = "Max phosp.",
                    x = as.numeric(personal_recommendation.df$min_pot),
                    y = as.numeric(personal_recommendation.df$max_pho),
                    xend = as.numeric(personal_recommendation.df$max_pot),
                    yend = as.numeric(personal_recommendation.df$max_pho))) +
        geom_segment(color = MyPalette[3], size = 0.6,
                    aes(x = as.numeric(personal_recommendation.df$min_pot),
                    y = as.numeric(personal_recommendation.df$min_pho),
                    xend = as.numeric(personal_recommendation.df$min_pot),
                    yend = as.numeric(personal_recommendation.df$max_pho))) +
        geom_segment(color = MyPalette[4], size = 0.6,
                    aes(x = as.numeric(personal_recommendation.df$max_pot),
                    y = as.numeric(personal_recommendation.df$min_pho),
                    xend = as.numeric(personal_recommendation.df$max_pot),
                    yend = as.numeric(personal_recommendation.df$max_pho))) +        
        labs(x = "Potassium intake (mg/d)", y = "Phosphorous intake (mg/d)") +
        annotate("pointrange",x = avg_k, y = avg_f, ymin = min_f, ymax = max_f, xmin = min_k, xmax = max_k, size = 0.1, colour = "black") +
        scale_colour_gradient2(low = "#FFFFFF",
                               high = "#0096FF",
                               midpoint = 0.75,
        ...
        guides(colour = guide_colorbar(title = "Probability % of reaching target concentrations", title.theme = element_text(size=7), label.theme = element_text(size=7), title.position = "top", barwidth = unit(80,"mm"))) +
        ggtitle("Personal intake limits inferred from matching concentration boundary\npredictions on the right, as highlighted by corresponding colors") +
        geom_segment(aes(xend = avg_k+60, x = 3550, yend = avg_f-30, y = 120), color="#333333", size=0.15) +
        geom_segment(aes(xend = 5700, x = 4500, yend = 125, y = 125), color="#333333", size=0.15, arrow = arrow(type = "closed", length = unit(0.06, "inches"))) +
        annotate("label", x = 3500, y = 115, label = "Point of observed intake that produced\nthe concentrations with dashed lines", size=2.5, vjust = "middle")    
      ...
      pk_plot <- ggplot(concs_minmax.df, aes(x = pk, group=factor(color_order))) +
        ...
        theme_bw() +
        theme(axis.text.x = element_text(size=6), axis.title = element_text(size=8), plot.title = element_text(size=9), axis.text.y = element_blank()) 
        #theme(text = element_text(family = "Times-Roman")) +
      ...
      palb_plot <- ggplot(concs_minmax.df, aes(x = palb, color=factor(color_order), group=factor(color_order))) +
        ...
        theme_bw() +
        theme(legend.position="bottom", legend.title = element_blank(), axis.text.x = element_text(size=6), axis.title = element_text(size=8), plot.title = element_text(size=9), axis.text.y = element_blank())
        #theme(text = element_text(family = "Times-Roman")) +
      ...
      #reasoning_plot <- grid.arrange(grobs=list(intake_plot,pk_plot,fppi_plot,palb_plot),layout_matrix = lay,padding=unit(0,"cm"),top=textGrob("Personally recommended intake range and the corresponding concentration predictions", gp = gpar(fontsize = 10)))
      reasoning_plot <- grid.arrange(grobs=list(intake_plot,pk_plot,fppi_plot,palb_plot),layout_matrix = lay,padding=unit(0,"cm"))
...
# Saving to TIFF only works from console
#library(Cairo)
#Cairo(file = "figures/Fig3.tiff", type = "tiff", width = 5, height = 5, bg="white", units = "in", res = 500)
#grid.arrange(intake_plot, pk_plot, fppi_plot, palb_plot, layout_matrix = lay, padding=0)
#dev.off()

```

## Personal recommendations for predicted reactions

```{r cross-validation-recommendations, eval=TRUE, include=TRUE, fig.align="center", out.height="\\textheight", out.width="100%", echo=FALSE, fig.cap="The figure shows personal recommendations of potassium and phosphorous intake ($\\hat{Q}^{min}-\\hat{Q}^{max}$) based on cross-validation predictions of nutrition effects, $\\hat{b}_k$. Other parameters of the figure are similar to Supplementary Fig. S4. The figure is plotted with ggplot2 package for R language (v 3.4.1, https://ggplot2.tidyverse.org)."}
knitr::include_graphics("figures/recommendation_plot_cross-validation_X50_B50.pdf")
```

```{r nrmse_plot, echo=FALSE, fig.cap="Figure shows normalized root mean square error (NRMSE) of the two-level model for each patient. Blue points indicate the in-sample error and red points indicate the error from cross-validation prediction. The lines between the points highlight the increased error between in-sample modelling and the prediction. Alternating colors of the lines denote the folds of the cross-validation. The figure is plotted with ggplot2 package for R language (v 3.4.1, https://ggplot2.tidyverse.org)."}
knitr::include_graphics("figures/nrmse_plot.pdf")
```
...
twolvl_fit <- mebn.get_localfit(target_name = "pk_fppi_palb", local_model_cache = "models/BLMM_gamma_mv_cross/two_levels/noncentered/")

twolvl_fit

p <- mebn.multivariate_dens_overlays("models/BLMM_gamma_mv_cross/two_levels/noncentered/", assumedtargets, dialysis) 
p

sumstats <- summary(twolvl_fit, pars="beta", probs=c(0.05,0.95))

sumstats$summary

assumedpredictors_fat_epros$Name # [13] "kalsium" 

eff <- na.omit(sumstats$summary[,9])
c(min(eff),mean(eff),max(eff))  # 5.588204  580.998852 8609.589616

rhat <- na.omit(sumstats$summary[,10])
c(min(rhat),mean(rhat),max(rhat)) # 0.9994999 1.0090897 1.7160137

mcmc_trace(twolvl_fit, pars = c("beta[1,1]"))

#                  mean    se_mean         sd        2.5%         25%          50%         75%      97.5%     n_eff     Rhat
#beta[3,13]  0.33476956 1.15207749  7.9343515 -15.6253793 -4.90603184 -0.240055853  6.12735050 15.5277882 47.430695 1.110139

# [1] "verenrasvojenlaakitys" "diabeteslaakitys"      "aktdvit"               "fosforinsitoja"        "renavit"               "hhepros"              
# [7] "rasvaepros"            "mufaepros"             "pufaepros"             "protepros"             "safaepros"             "kuitu"                
#[13] "protgkg"               "sukupuoli"             "ekcalkg"               "kalsium"               "fosfori"               "kalium"               
#[19] "suola"                 "natrium"               "vesi"                  "dvit"     


library(ggplot2)

samples <- rstan::extract(twolvl_fit, pars = c("beta"))
kalium_palb <- samples$beta[,3,18]

mean_kalium_palb <- mean(kalium_palb)
lCI <- quantile(kalium_palb, probs=0.05)   
uCI <- quantile(kalium_palb, probs=0.95)   

ggplot(as.data.frame(kalium_palb), aes(x=kalium_palb)) +
  geom_density(fill="lightblue") + 
  geom_vline(xintercept = mean_kalium_palb, color = "green") + 
  geom_vline(xintercept = lCI) +
  geom_vline(xintercept = uCI) 


palb <- mebn.localsummary_from_two_level_multivariate(twolvl_fit, 3)

cbind(palb$fixef_lCI, palb$fixef, palb$fixef_uCI)

ms <- summary(twolvl_fit, pars=c(paste0("beta_Intercept"), paste0("beta"), paste0("sigma_b_g"), paste0("sigma_b_s")), probs=c(0.05, 0.95), na.rm = TRUE)
beta <- summary(twolvl_fit, pars=c(paste0("beta")), probs=c(0.05, 0.95), na.rm = TRUE)
beta

fit <- mebn.get_localfit(target_name = "pk_fppi_palb", local_model_cache = "models/BLMM_gamma_mv_cross/two_levels/noncentered/")
group_id <- 
