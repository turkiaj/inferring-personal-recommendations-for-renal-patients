...
recommendation_plot <- grid.arrange(grobs=list(pot_plot,pho_plot,pk_plot,fppi_plot,palb_plot),top=textGrob("Personally recommended intake ranges and corresponding concentration predictions", gp = gpar(fontsize = 11)),ncol=5,nrow=1,padding=unit(0,"cm"),widths=c(1.32,1,1,1,1))
...
#  person_id <- 28
  ...
  if (file.exists(accepted_intake)) {
    ...
    else
    {
      ...
      reasoning_plot <- grid.arrange(grobs=list(intake_plot,pk_plot,fppi_plot,palb_plot),layout_matrix = lay,padding=unit(0,"cm"),top=textGrob("Personally recommended intake range and the corresponding concentration predictions", gp = gpar(fontsize = 11)))
