...
#   ungroup() %>%
#   arrange(desc(personal_variation)) %>%
#   select(effect, personal_min, lCI_min, uCI_min, personal_max, lCI_max, uCI_max, personal_variation) %>%
#   distinct()
...
top_effect_summary <- effect_summary %>%
  ...
  select(effect, personal_min, personal_max, lCI_min, uCI_min, lCI_max, uCI_max, personal_abs, personal_variation) %>%
  distinct()
...
kable(top_effect_summary[1:20, c(10,2,3)], "latex", escape = T, booktabs = T, row.names = FALSE, digits = 3, col.names = c("Effect", "Sample min", "Sample max")) %>%
    kable_styling(latex_options = "HOLD_position") %>%
    kable_styling(latex_options = c("basic", "condensed"), full_width = FALSE) %>%
