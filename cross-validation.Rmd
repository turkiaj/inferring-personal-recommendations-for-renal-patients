...
# Assume b_est is a vector of estimated personal coefficients for the new patient
...
for (person_id in 1:4)
{
  ...
  for (c in 1:dim(assumedtargets)[1])
  {
    ...
    predictor_names <- assumedpredictors_fat_epros$Name
    
    pe <- mebn.personal_effects_from_multivariate(obsfit, c, person_id)
    
    p_int <- extract(obsfit, pars = c(paste0("personal_intercept[",person_id,",",c,"]")))
    p_int_mean <- mean(unlist(p_int))
    
    b_est <- cbind(b_est, p_int_mean)
  
    for (p in 1:length(predictor_names))
    {
      predictor_name <- predictor_names[p]
      
      #vertex_name <- paste0("personal_", predictor_name, "_", target_name)
      #print(vertex_name)
      #print(pe$personal_effect[p])
      
      b_est <- cbind(b_est, pe$personal_effect[p])
    }
  }
  
  # Estimate how far b_est is from distribution of previously learned reactions L_s 
  # with Mahalanobis distance
  
  C <- L_s %*% t(L_s)
  inv_C <- solve(C) # 69 x 69
  b_est <- as.vector(b_est)
  
  Mahalanobis_distance <- sqrt(t(b_est) %*% inv_C %*% b_est)
...
saveRDS(md_summary, "model_evaluation/mahalanobis_distance_1.rds")
...
md_summary <- readRDS("model_evaluation/mahalanobis_distance_1.rds")
