...
params <- within(list(),
                 {
                   ...
                   L_g <- colMeans(pmodel$L_g)                             # cholesky_factor_corr[v*k]
                   stacked_sigma_b_g <- colMeans(pmodel$stacked_sigma_b_g) # vector[v*k]
...
pred_result <- stan(file=stan_model_file, warmup = 100, iter=400, chains=4, chain_id=1L, algorithm="NUTS", control = list(adapt_delta = 0.99, max_treedepth = 15), data=params, seed=483892929)
