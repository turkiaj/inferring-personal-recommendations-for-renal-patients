...
error_analysis.df$patient_id <- as.factor(seq(1:37))
...
nrmse_plot <- ggplot(error_analysis.df) +
  ...
  geom_segment(aes(y = reorder(subject, patient_id), yend = subject, x = avg_error, xend = avg_cv_error, group = patient_id, color = fold_color), linetype = "solid", show.legend = FALSE) +
  theme_bw() +
...
nrmse_plot <- ggplot(error_analysis.df) +
  geom_point(aes(y = patient_id, x = avg_error), color = "#799fcb") +
  geom_point(aes(y = patient_id, x = avg_cv_error), color = "#f9665e") +
  geom_segment(aes(y = patient_id, yend = patient_id, x = avg_error, xend = avg_cv_error, group = patient_id, color = fold_color), linetype = "solid", show.legend = FALSE) +
  theme_bw() +
  scale_fill_viridis(discrete = TRUE) + 
  theme(text = element_text(family = "Times New Roman")) +
  labs(x = "NRMSE", y = "Patient") 
...
library(Cairo)
Cairo(file = "figures/FigS5.tiff", type = "tiff", width = 4, height = 4, bg="white", units = "in", res = 300)
print(nrmse_plot)
dev.off()
